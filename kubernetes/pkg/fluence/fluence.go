...
import (
	...
	"k8s.io/kubernetes/pkg/scheduler/framework/plugins/fluence/logger"
	...
	fcore "k8s.io/kubernetes/pkg/scheduler/framework/plugins/fluence/core"
	fgroup "k8s.io/kubernetes/pkg/scheduler/framework/plugins/fluence/group"
	flabel "k8s.io/kubernetes/pkg/scheduler/framework/plugins/fluence/labels"
...
func (fluence *Fluence) PostFilter(
	...
	fluence.handle.IterateOverWaitingPods(func(waitingPod framework.WaitingPod) {
	...
	if fluence.podGroupBackoff != nil {
		pods, err := fluence.handle.SharedInformerFactory().Core().V1().Pods().Lister().Pods(pod.Namespace).List(
