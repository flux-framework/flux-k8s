...
def gather_outputs(outdir):
    ...
    for file in os.listdir(outdir):
        if file.startswith("qmcpack") and file.endswith(".out"):
            ...
            for line in lines:
                if "for scheduler" in line:
                    ...
                    sline = line.strip().replace(",", "").split(" ")
                    spp = int(sline[3])
                elif "target walkers                =" in line:
                    walkers = float(line.strip().split("=")[-1])
                elif "Total Execution time" in line:
                    texec = float(line.strip().split(" ")[-2])
...
def plot_outputs(df, plotname, slot_per_pod):
    ...
    plt.title("QMCPACK MPIJob startup time")
    plt.legend([], [], frameon=False)
    ax.set_xlabel("MPI Ranks", fontsize=20)
    ...
    plt.title("QMCPACK MPIJob runtime")
    plt.legend([], [], frameon=False)
    ax.set_xlabel("MPI Ranks", fontsize=20)
    ...
    plt.title("QMCPACK MPIJob end-to-end time")
    plt.legend([], [], frameon=False)
    ax.set_xlabel("MPI Ranks", fontsize=20)
    ...
    plt.title("QMCPACK Unix real time")
    plt.legend([], [], frameon=False)
    ax.set_xlabel("MPI Ranks", fontsize=20)
...
def main():
    parser = argparse.ArgumentParser(description="Process QMCPACK outputs")
    parser.add_argument(
        "-output_dir",
        "--output_dir",
        required=True,
